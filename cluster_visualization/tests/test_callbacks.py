...
class TestMainPlotCallbacks(unittest.TestCase):
    ...
    def test_create_traces_with_modular_creator(self):
        ...
        result = self.main_plot_callbacks.create_traces(self.test_data, True, False, None, True)
    ...
    def test_create_traces_fallback(self):
        ...
        result = fallback_callbacks.create_traces(self.test_data, True, False, None, True)
    ...
    def test_calculate_filtered_count(self):
        ...
        expected = len(
            self.test_data["merged_data"][self.test_data["merged_data"]["SNR_CLUSTER"] >= 5.0]
        )
        ...
        expected = len(
            self.test_data["merged_data"][self.test_data["merged_data"]["SNR_CLUSTER"] <= 10.0]
        )
        self.assertEqual(count, expected)
        ...
        expected = len(
            filtered_data[
                (filtered_data["SNR_CLUSTER"] >= 5.0) & (filtered_data["SNR_CLUSTER"] <= 10.0)
            ]
        )
        self.assertEqual(count, expected)
    ...
    def test_extract_existing_mer_traces(self):
        ...
        current_figure = {
            "data": [
                {"name": "Regular Data", "x": [1, 2, 3], "y": [1, 2, 3]},
                {
...
class TestMERCallbacks(unittest.TestCase):
    ...
    def test_extract_zoom_ranges(self):
        ...
        relayout_data2 = {"xaxis.range": [12.5, 13.5], "yaxis.range": [-0.5, 0.5]}
