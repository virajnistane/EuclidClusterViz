...
class TestTraceCreator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.colors_list_transparent = ['rgba(255,0,0,0.3)', 'rgba(0,0,255,0.3)', 'rgba(0,255,0,0.3)', 'rgba(255,165,0,0.3)', 'rgba(128,0,128,0.3)']
        
        # Mock MER handler
        self.mer_handler = MagicMock()
        
        self.trace_creator = TraceCreator(self.colors_list, self.colors_list_transparent, self.mer_handler)
    ...
    def test_create_cluster_traces_tiles(self):
        ...
        traces = self.trace_creator._create_cluster_traces_tiles(
            self.test_data['tile_data']
        )
...
class TestTraceCreatorEdgeCases(unittest.TestCase):
    ...
    def test_invalid_snr_bounds(self):
        ...
        filtered = self.trace_creator._filter_data_by_snr(
            ...
            snr_threshold_lower=10.0,
            snr_threshold_upper=5.0
        )
    ...
    def test_missing_columns(self):
        ...
        try:
            traces = self.trace_creator._create_cluster_traces_merged(incomplete_data, show_polygons=False)
            self.assertIsInstance(traces, list)
