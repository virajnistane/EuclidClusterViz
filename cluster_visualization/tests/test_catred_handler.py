...
class TestCATREDHandler(unittest.TestCase):
    ...
    def test_get_radec_mertile_polygon_fallback(self):
        ...
        if not data['catred_info'].empty:
            ...
            if ('polygon' in data['catred_info'].columns and 
                data['catred_info'].loc[mertile_id, 'polygon'] is not None):
                self.assertIsInstance(result, dict)
    ...
    def test_load_fits_data(self, mock_fits_open, mock_open):
        ...
        # Mock FITS data
        mock_data = MagicMock()
        mock_data.columns.names = ['RIGHT_ASCENSION', 'DECLINATION', 'PHZ_MODE_1', 'PHZ_70_INT', 'PHZ_PDF']
