...
class FigureManager:
    ...
    def create_empty_figure(
        self, free_aspect_ratio: bool = True, show_initial_message: bool = True
    ) -> go.Figure:
        """
        ...
        # Configure aspect ratio
        xaxis_config, yaxis_config = self._get_axis_config(
            free_aspect_ratio, visible=not show_initial_message
        )
    ...
    def _get_axis_config(self, free_aspect_ratio: bool, visible: bool = True) -> Tuple[Dict, Dict]:
        ...
        if free_aspect_ratio:
            ...
            xaxis_config = dict(
                visible=visible, autorange="reversed"
    ...
    def _setup_layout(self, fig: go.Figure, algorithm: str) -> None:
        ...
        fig.update_layout(
            ...
            showlegend=True,
            legend=dict(
                title="Legend", orientation="v", xanchor="left", x=1.01, yanchor="top", y=1
            ),
        )
    ...
    def _has_valid_zoom_data(self, relayout_data: Dict) -> bool:
        ...
        has_x_zoom = (
            "xaxis.range[0]" in relayout_data and "xaxis.range[1]" in relayout_data
        ) or "xaxis.range" in relayout_data
        has_y_zoom = (
            "yaxis.range[0]" in relayout_data and "yaxis.range[1]" in relayout_data
        ) or "yaxis.range" in relayout_data
