...
class PHZCallbacks:
    ...
    def __init__(self, app, mer_handler=None, main_app=None, trace_creator=None):
        """
        ...
        Args:
            ...
            main_app: Main ClusterVisualizationApp instance for fallback access (optional)
            trace_creator: TraceCreator instance for accessing MER data (optional)
        """
        self.app = app
    ...
    def setup_callbacks(self):
        ...
        print("Debug: Setting up PHZ callbacks...")
        self._setup_initial_phz_plot()
        self._setup_phz_pdf_callback()
        print("Debug: PHZ callbacks setup complete")
    
    def _setup_initial_phz_plot(self):
        """Setup initial empty PHZ plot"""
        @self.app.callback(
            Output('phz-pdf-plot', 'figure'),
            [Input('render-button', 'n_clicks')],
            prevent_initial_call=False
        )
        def create_initial_phz_plot(n_clicks):
            """Create initial empty PHZ plot"""
    ...
    def _setup_phz_pdf_callback(self):
        ...
        def update_phz_pdf_plot(clickData):
            print(f"Debug: PHZ Click callback triggered with clickData: {clickData}")
            ...
            elif self.trace_creator and hasattr(self.trace_creator, 'current_mer_data'):
                current_mer_data = self.trace_creator.current_mer_data
            elif self.main_app and hasattr(self.main_app, 'current_mer_data'):
                current_mer_data = self.main_app.current_mer_data
            elif hasattr(self, 'current_mer_data'):
                current_mer_data = self.current_mer_data
            
            print(f"Debug: Current MER data source check:")
            print(f"  - mer_handler.current_mer_data: {hasattr(self.mer_handler, 'current_mer_data') if self.mer_handler else 'No mer_handler'}")
            print(f"  - trace_creator.current_mer_data: {hasattr(self.trace_creator, 'current_mer_data') if self.trace_creator else 'No trace_creator'}")
            print(f"  - main_app.current_mer_data: {hasattr(self.main_app, 'current_mer_data') if self.main_app else 'No main_app'}")
            print(f"  - current_mer_data: {current_mer_data is not None}")
