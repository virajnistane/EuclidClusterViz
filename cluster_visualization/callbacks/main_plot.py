...
class MainPlotCallbacks:
    ...
    def _setup_mask_overlay_callback(self):
        ...
        @self.app.callback(
            Output('cluster-plot', 'figure', allow_duplicate=True),
            [Input('healpix-mask-switch', 'value')],
            [State('cluster-plot', 'figure'),
             State('cluster-plot', 'relayoutData'),
             State('mosaic-enable-switch', 'value'),
             State('algorithm-dropdown', 'value')],
        ...
        def render_mask_overlay(overlay_mask, current_figure, relayout_data, mosaic_enabled, algorithm):
            """Render mosaic images when button is clicked"""
            try:
                print(f"🔍 Mask overlay callback triggered! overlay_mask={overlay_mask}, mosaic_enabled={mosaic_enabled}")
                print(f"   -> relayout_data keys: {list(relayout_data.keys()) if relayout_data else None}")
                print(f"   -> algorithm: {algorithm}")
                print(f"   -> current_figure exists: {current_figure is not None}")

                if not overlay_mask:
                    print(f"   -> Returning early: overlay_mask is {overlay_mask}")
                    return current_figure
