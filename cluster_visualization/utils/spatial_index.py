...
class SpatialIndex:
    ...
    def query_radius(self, ra_center: float, dec_center: float, 
    ...
    def query_multiple_radius(self, ra_centers: np.ndarray, dec_centers: np.ndarray,
                              radius_deg: float) -> List[np.ndarray]:
        """
    ...
    def query_box(self, ra_min: float, ra_max: float, 
        ...
        """
        mask = ((self.ra >= ra_min) & (self.ra <= ra_max) &
...
class CATREDSpatialIndex:
    ...
    def __init__(self, ra: np.ndarray, dec: np.ndarray, subsample_threshold: int = 100000):
        ...
        if self.n_original > subsample_threshold:
            ...
            print(f"CATRED spatial index: subsampled {len(indices):,} from {self.n_original:,} points")
        else:
            self.index = SpatialIndex(ra, dec)
            self.subsampled = False
            print(f"CATRED spatial index: using all {self.n_original:,} points")
    
    def check_proximity_batch(self, ra_array: np.ndarray, dec_array: np.ndarray,
                              radius_deg: float = 0.1) -> np.ndarray:
        """
    ...
    def check_proximity_single(self, ra: float, dec: float, 
                               radius_deg: float = 0.1) -> bool:
        """
