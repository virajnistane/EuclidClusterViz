...
class MemoryManager:
    ...
    def cleanup_if_needed(self, cache_dict: Dict[str, Any]) -> bool:
        ...
        if current_memory > self.warning_threshold_bytes:
            ...
            print(f"⚠️  Memory threshold exceeded: {stats['rss_gb']:.2f} GB / {self.max_memory_bytes / 1024**3:.2f} GB")
    ...
    def print_cache_report(self, cache_dict: Dict[str, Any]) -> None:
        ...
        print(f"  Available:      {mem_stats['available_gb']:.1f} GB ({100 - mem_stats['system_percent']:.1f}% free)")
        print(f"\nProcess Memory:")
        ...
        print(f"  Usage:          {(mem_stats['rss_gb'] / (self.max_memory_bytes / 1024**3)) * 100:.1f}%")
        
        if cache_stats['items']:
            print(f"\nCached items (by recency):")
            for item in cache_stats['items'][:5]:  # Show top 5
                age_str = f"{item['age_seconds']/60:.1f} min" if item['age_seconds'] > 60 else f"{item['age_seconds']:.0f} sec"
